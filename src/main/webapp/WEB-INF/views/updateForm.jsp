...
<body>
	...
	<form name="Register" method="post" action="update">
		...
		<p> 취미 : 중드 <input type="checkbox" name="rhobby" value="중드" <c:if test="${fn:contains(edit.rhobby,'중드')}">checked</c:if>> 
				  조립 <input type="checkbox" name="rhobby" value="조립" <c:if test="${fn:contains(edit.rhobby,'조립')}">checked</c:if>>
				  음악 <input type="checkbox" name="rhobby" value="음악" <c:if test="${fn:contains(edit.rhobby,'음악')}">checked</c:if>>
